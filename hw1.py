...
def check_referential_integrity(conn, table_name: str, fks: list) -> str:
    ...
    for fk in fks:
        ...
        cur.execute(q_count_all)
        (cnt_all,) = cur.fetchone()
        log_ri_sql(q_count_all)
        cur.execute(q_count_join)
        (cnt_join,) = cur.fetchone()
        log_ri_sql(q_count_join)
...
def check_normalization(conn, table_name, pk, cols):
    ...
    for x in non_pk:
        # does X repeat?
        n, dx = run_and_log(
            cur, f"SELECT COUNT(*), COUNT(DISTINCT {x}) FROM {table_name};"
        )
        if dx == n:
            continue
...
def output_results(testcase, refint, normal):
    ...
    all_ref = reduce(collect_results, refint.values())
    all_norm = reduce(collect_results, normal.values())

    table_results = []
    for key in normal.keys():
        table_results.push(key, refint[key], normal[key])
    ...
    with open(file, "a", encoding="utf-8") as f:
        ...
        for table in table_results:
            f.write(f"{table[0]}                {table[1]}           {table[2]}\n")

        f.write(f"DB referential integrity: {all_ref}\nDB normalized: {all_norm}\n\n\n")
    return
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        output_results(Path(schema_file), refint, normal)
